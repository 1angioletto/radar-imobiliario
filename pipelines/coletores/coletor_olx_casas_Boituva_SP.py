...
# Coletor específico para CASAS à venda em Boituva.
# Mantém o mesmo layout de colunas do coletor de terrenos para carregar na mesma raw_olx_historico.
BASE_URL = "https://www.olx.com.br/imoveis/casas/compra/estado-sp/regiao-de-sorocaba/boituva"
...
def extract_location_from_text(text: str) -> tuple[str, str, str]:
    ...
    for line in lines:
        match = re.search(r"^(.*?),\s*boituva\s*-\s*sp$", line, flags=re.IGNORECASE)
        if match:
...
def extract_detail(page, url: str) -> Optional[dict]:
    ...
    endereco_texto = ""
    for line in lines:
        if "boituva" in line.lower() and "sp" in line.lower():
            endereco_texto = line
            break
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        for i, url in enumerate(links[:max_items], start=1):
            try:
                print(f"[{i}/{min(len(links), max_items)}] Coletando anúncio: {url}")
