...
TARGET_CITY = "Lagarto"
TARGET_CITY_LOWER = "lagarto"
TARGET_STATE = "SE"

BASE_URL = "https://www.olx.com.br/imoveis/terrenos/compra/lotes/estado-se/sergipe/outras-cidades/lagarto"
...
def extract_location_from_text(text: str) -> tuple[str, str, str]:
    ...
    for line in lines:
        pattern = rf"^(.*?),\s*{TARGET_CITY_LOWER}\s*-\s*{TARGET_STATE.lower()}$"
...
def extract_detail(page, url: str) -> Optional[dict]:
    ...
    for line in lines:
        ...
        if TARGET_CITY_LOWER in line_lower and TARGET_STATE.lower() in line_lower:
            endereco_texto = line
            break
